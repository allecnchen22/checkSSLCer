...
public class KeyPinStore {
    ...
    private KeyPinStore() throws CertificateException, IOException, KeyStoreException, NoSuchAlgorithmException, KeyManagementException{
        ...
        //InputStream caInput = new BufferedInputStream(MainActivity.context.getAssets().open("load-der.crt"));
        Certificate ca;
        try {
            ca = cf.generateCertificate(caInput);
            Log.d("AAA",ca.getPublicKey().toString());
            
            //System.out.println("ca=" + ((X509Certificate) ca).getSubjectDN());
            //Log.d("AAA","ca=" + ((X509Certificate) ca).getSubjectDN());
        } finally {
            caInput.close();
        }
        ...
        InputStream allInput = new BufferedInputStream(MainActivity.context.getAssets().open("client_ts.bks"));
        keyStore.load(allInput, new char[]{'1','2','3','4','5','6'});
